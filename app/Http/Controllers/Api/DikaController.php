...
class DikaController extends Controller
{
    ...
    public function GetProductItemInStation($station_id)
    {
        $product_assigned_stations = DB::table("product_assigned_station")
            ->where("station_id", $station_id)
            ->whereIn('product_id', $this->productIdList)
            ->get();
        ...
        foreach ($product_assigned_stations as $product_assigned_station) {
            ...
            if($order_item->order_index == $this->max_order_index){
                $product = $this->GetProduct($product_id);
    ...
    public function getOrderAllStation()
    {
    ...
    public function GetOrderItemsAllStation($order_id)
    {
        ...
        foreach ($product_stations as $product_station) {
            ...
            $product_station->items = $this->GetProductItemInStation($product_station->id);
    ...
    public function create()
    {
        ...
        if ($pos_mode == "New Order") {
            $postdata["queue_number"] = $this->getQueueNumber();
            $postdata["takeaway_number"] = $this->getTakeAwayNumber();
            $postdata["po_number"] = uniqid();
        }
        ...
        $this->sendNotificationToKitchen("New Order Created!", "Queue Number" . $postdata["queue_number"]);
    ...
    public function getPrintKitchenData()
    {
        $order_id = Input::get("order_id");
        $strQuery = "SELECT * FROM order_detail WHERE COALESCE(order_index,0) <> 0 AND order_id = $order_id AND order_index = (SELECT MAX(order_index) FROM order_detail WHERE order_id = $order_id AND status <> 'Void Order')";
        $query = DB::select($strQuery);
